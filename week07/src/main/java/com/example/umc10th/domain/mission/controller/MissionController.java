...
public class MissionController {
    ...
    @Operation(
            ...
            description = "Request Body에서 사용자 ID를 받아 진행 중인 미션 목록을 오프셋 기반 페이지네이션으로 반환합니다."
    )
    @PostMapping("/in-progress")
    public ApiResponse<MissionResDTO.MissionListDTO> getMyInProgressMissions(
            @RequestBody @Valid MissionReqDTO.MyInProgressDTO request) {
        return ApiResponse.onSuccess(SuccessCode.MISSION_FOUND,
                missionService.getMyInProgressMissions(request));
    }
}
