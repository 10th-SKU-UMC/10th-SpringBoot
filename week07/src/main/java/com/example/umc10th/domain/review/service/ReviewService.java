...
public class ReviewService {
    ...
    /**
     * 내가 생성한 리뷰 커서 기반 페이지네이션 조회 (사진 제외)
     * sortBy: "id" — ID 내림차순, "rating" — 별점 내림차순(동점은 ID 내림차순)
     * cursor: sortBy=id 일 때 마지막 reviewId, sortBy=rating 일 때 "별점_reviewId" 형식
     */
    public ReviewResDTO.ReviewCursorListDTO getMyReviewsCursor(Long memberId, String cursor,
                                                               int size, String sortBy) {
        findMemberById(memberId);
        Pageable pageable = PageRequest.of(0, size + 1); // hasNext 판별용으로 1개 더 조회

        List<Review> reviews;
        if ("rating".equals(sortBy)) {
            if (cursor == null || cursor.isBlank()) {
                reviews = reviewRepository.findByMemberIdOrderByRatingDesc(memberId, pageable);
            } else {
                String[] parts = cursor.split("_");
                Integer cursorRating = Integer.parseInt(parts[0]);
                Long cursorId = Long.parseLong(parts[1]);
                reviews = reviewRepository.findByMemberIdCursorByRatingDesc(
            ...
            if (cursor == null || cursor.isBlank()) {
                ...
                reviews = reviewRepository.findByMemberIdCursorByIdDesc(memberId, cursorId, pageable);
            }
        }
        ...
        if (hasNext && !reviews.isEmpty()) {
            ...
            nextCursor = "rating".equals(sortBy)
                    ? last.getRating() + "_" + last.getId()
                    : String.valueOf(last.getId());
        }

        return ReviewConverter.toReviewCursorListDTO(reviews, nextCursor, hasNext);
    }
